...
def on_message(client, userdata, msg):
    ...
    if float(m_decode.split(': ')[1]) < gas_weight_THR:
        ic("Threshold warning! The gas weight is : " + m_decode.split(': ')[1] + ', you need to replace baloon')
        send_msg(client, topic_alarm, "Threshold warning! The gas weight is: " + m_decode.split(': ')[1] + ', you need to replace baloon')
...
def main():    
    ...
    client.subscribe(comm_topic+'WD')
    try:
        while conn_time==0:           
